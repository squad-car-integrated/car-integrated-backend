...
export class InMemoryServiceProductsRepository implements ServiceProductsRepository {
    async deleteManyByServiceId(serviceId: string) {
        const serviceProducts = this.items.filter(item => item.serviceId.toString() !== serviceId)
