...
describe("Fetch recent products", () => {
    ...
    it("Should be able to fetch recent products", async () => {
        await inMemoryProductsRepository.create(makeProduct({createdAt: new Date(2022,0,20)}))
        await inMemoryProductsRepository.create(makeProduct({createdAt: new Date(2022,0,18)}))
        await inMemoryProductsRepository.create(makeProduct({createdAt: new Date(2022,0,23)}))
