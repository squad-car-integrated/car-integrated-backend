...
type GetOwnerByIdUseCaseResponse = Either<ResourceNotFoundError, {owner: Owner}>
export class GetOwnerByIdUseCase { 
    constructor(
        private ownerRepository: OwnersRepository,
    ){}
    async execute({id}: GetOwnerByIdUseCaseRequest) : Promise<GetOwnerByIdUseCaseResponse> {
