...
type GetOwnerByEmailUseCaseResponse = Either<ResourceNotFoundError, {owner: Owner}>
export class GetOwnerByEmailUseCase { 
    constructor(
        private ownerRepository: OwnersRepository,
    ){}
    async execute({email}: GetOwnerByEmailUseCaseRequest) : Promise<GetOwnerByEmailUseCaseResponse> {
