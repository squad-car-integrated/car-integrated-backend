...
type GetAutomobileByIdUseCaseResponse = Either<ResourceNotFoundError, {automobile: Automobile}>
export class GetAutomobileByIdUseCase { 
    constructor(
        private automobileRepository: AutomobilesRepository,
    ){}
    async execute({id}: GetAutomobileByIdUseCaseRequest) : Promise<GetAutomobileByIdUseCaseResponse> {
