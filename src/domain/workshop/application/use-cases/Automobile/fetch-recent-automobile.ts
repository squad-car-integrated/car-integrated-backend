...
type FetchRecentAutomobilesUseCaseResponse = Either<null,{automobiles: Automobile[]}>
@Injectable()
export class FetchRecentAutomobilesUseCase { 
    constructor(
        private automobileRepository: AutomobilesRepository,
    ){}
    async execute({page}: FetchRecentAutomobilesUseCaseRequest) : Promise<FetchRecentAutomobilesUseCaseResponse> {
        const automobiles = await this.automobileRepository.findManyRecent({page})
        return right({
            automobiles
        })
    }
