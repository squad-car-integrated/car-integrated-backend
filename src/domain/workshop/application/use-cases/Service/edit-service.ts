...
export class EditServiceUseCase {
    ...
    async execute({
        ...
        const currentServiceProducts =
            await this.serviceProductsRepository.findManyByServiceId(serviceId)
        const currentServiceEmployees =
            await this.serviceEmployeesRepository.findManyByServiceId(serviceId)

        const serviceProductList = new ServiceProductList(
            currentServiceProducts,
        )
        const serviceEmployeeList = new ServiceEmployeeList(
            currentServiceEmployees,
        )

        const serviceProducts = products.map((productAndQuantity) => {
            const oldId = currentServiceProducts
                .find(
                    (product) =>
        ...
        const serviceEmployees = employees.map((employeeId) => {
            const oldId = currentServiceEmployees
                .find(
                    (employee) => employee.employeeId.toString() == employeeId,
                )
                ?.id.toString()
            const newId = oldId
                ? new UniqueEntityID(oldId)
